...
def render_project_creation():
    ...
    with st.form("create_project_form"):
        ...
        project_description = st.text_area("Project Description (Optional)", help="Brief description of the project's purpose")
        language = st.selectbox("Language", ["en", "he", "multi"], help="Primary language of your documents")
        image_enrichment = st.checkbox("Enable Image Enrichment", help="Extract text from images and screenshots")
        ...
        if submitted:
            ...
            if not re.match(r'^[a-zA-Z0-9_\-\s]+$', project_name.strip()):
                validation_errors.append("Project Name can only contain letters, numbers, spaces, hyphens, and underscores")
            ...
            with st.spinner("Creating project..."):
                try:
                    ...
                    st.error("❌ Permission denied. Check that you have write access to the projects directory.")
                
                except OSError as e:
                    st.error(f"❌ File system error: {e}")
                    st.info("💡 This might be due to insufficient disk space or invalid characters in the project name.")
...
def render_config_editor_v2(project_path: Path):
    ...
    with st.form("config_form"):
        ...
        embedding_options = ["text-embedding-3-large", "text-embedding-ada-002", "bge-large-en-v1.5"]
        current_model = embedding_section.get("model", "text-embedding-3-large")
        model_index = embedding_options.index(current_model) if current_model in embedding_options else 0
